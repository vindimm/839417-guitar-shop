...
describe('Component: CartList', () => {
  it('should render correctly empty cart', () => {
    render(
    ...
    expect(screen.queryAllByTestId('cart-item-testid')).toHaveLength(0);
    expect(screen.getByText('В корзине сейчас нет товаров.')).toBeInTheDocument();
  });

  it('should render correctly 2 items in cart', () => {
    render(
      <HistoryRouter history={customHistory}>
        <Provider store={mockStore({
          CATALOG_DATA: {
    ...
    expect(screen.getAllByTestId('cart-item-testid')).toHaveLength(2);
    expect(screen.queryByText('В корзине сейчас нет товаров.')).not.toBeInTheDocument();
  });
});
